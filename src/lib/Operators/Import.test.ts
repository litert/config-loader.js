...
NodeTest.describe('Built-in Operator: Import', async () => {
    ...
    await NodeTest.it('[container-mode] should throw error if not circular loading a same file', async () => {
        ...
        await NodeAssert.rejects(() => loader.load('/config.json'), {
            message: 'Circular reference detected when importing from "/config.json".'
        });
        await NodeAssert.rejects(() => loader.load('/a.json'), {
            message: 'Circular reference detected when importing from "/a.json".'
        });
        await NodeAssert.rejects(() => loader.load('/b.json'), {
            message: 'Circular reference detected when importing from "/b.json".'
        });
        await NodeAssert.rejects(() => loader.load('/c.json'), {
            message: 'Circular reference detected when importing from "/c.json".'
        });
